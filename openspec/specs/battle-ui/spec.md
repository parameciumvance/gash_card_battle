...
### Requirement: 操作與決策互動
UI SHALL 只提供當下合法的操作(依狀態快照過濾);引擎要求決策(ChoiceRequired)時 MUST 以對話框呈現選項(目標選擇/庇護與否/受傷順序/付費重擲),並標明是哪位玩家決策。hotseat 下 SHALL 明確標示當前輪到哪位玩家行動。
...
#### Scenario: 庇護對話框
- **WHEN** 引擎發出庇護決策要求
- **THEN** 畫面彈出對話框列出可庇護的魔物與「不庇護」選項,標明由受傷方玩家選擇
